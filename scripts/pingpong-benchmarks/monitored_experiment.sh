...
while [ "$experiments" -ne 0 ] ; do
  ...
  jmeter -n -t $wd/scripts/pingpong-benchmarks/test-plan.jmx -Jpath=$wd/scripts/pingpong-benchmarks/results/monitored/${requests}_resp_time_run${run}.csv -Jrequests=${requests} -Jrampup=${rampup}
