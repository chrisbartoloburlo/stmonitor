...
class Mon(external: ConnectionManager, internal: Out[ExternalChoice1], max: Int, zvalue: Double)(implicit ec: ExecutionContext, timeout: Duration) extends Runnable {
	...
	def receiveExternalChoice1(internal: Out[ExternalChoice1], external: ConnectionManager, count: Int): TailRec[Unit] = {
		...
		external.receive() match {
			...
			case msg @ Help()=>
				labels.Help_5.counter+=1
				checkExternalChoice1Intervals()
				val cont = internal !! Help()_
				if (count < max) {
					sendHint_4(cont, external, count+1)
				} else { tailcall(sendHint_4(cont, external, 0)) }
			case msg @ Quit()=>
				labels.Quit_6.counter+=1
				checkExternalChoice1Intervals()
					internal ! msg; done()
			case _ => done()
		}
	...
	def sendHint_4(internal: In[Hint], external: ConnectionManager, count: Int): TailRec[Unit] = {
		internal ? {
			case msg @ Hint(_) =>
				external.send(msg)
				if (count < max) {
	...
	def checkExternalChoice1Intervals(): Unit = {
		...
		val (pmin_Help,pmax_Help) = calculateInterval(labels.Help_5.counter, labels.ExternalChoice1.counter)
		if(pmin_Help >= labels.Help_5.prob || pmax_Help <= labels.Help_5.prob) {
			if(!labels.Help_5.warn){
				println(f"[MON] **WARN** ?Help[${labels.Help_5.prob}] outside interval [$pmin_Help,$pmax_Help]")
				labels.Help_5.warn = true
			}
		} else {
			if(labels.Help_5.warn){
				println(f"[MON] **INFO** ?Help[${labels.Help_5.prob}] within interval [$pmin_Help,$pmax_Help]")
				labels.Help_5.warn = false
			}
		}
		val (pmin_Quit,pmax_Quit) = calculateInterval(labels.Quit_6.counter, labels.ExternalChoice1.counter)
		if(pmin_Quit >= labels.Quit_6.prob || pmax_Quit <= labels.Quit_6.prob) {
			if(!labels.Quit_6.warn){
				println(f"[MON] **WARN** ?Quit[${labels.Quit_6.prob}] outside interval [$pmin_Quit,$pmax_Quit]")
				labels.Quit_6.warn = true
			}
		} else {
			if(labels.Quit_6.warn){
				println(f"[MON] **INFO** ?Quit[${labels.Quit_6.prob}] within interval [$pmin_Quit,$pmax_Quit]")
				labels.Quit_6.warn = false
			}
		}
	}
