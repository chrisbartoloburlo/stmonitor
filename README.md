...
* Christian Bartolo Burlò, Adrian Francalanza, Alceste Scalas, Catia Trubiani and Emilio Tuosto. *[Towards Probabilistic Session-Type Monitoring](https://link.springer.com/chapter/10.1007/978-3-030-78142-2_7)*. ([Pesentation video](https://www.youtube.com/watch?v=7ncHqpgTjjc) and [Tool demo](https://www.youtube.com/watch?v=_NlaNk6nphQ)) **NOTE:** *to access the implementation of the probabilistic monitors switch branch to `pstmonitor`.*
...
To generate a monitor, [`STParser`](https://github.com/chrisbartoloburlo/ecoop2021-artifact/blob/main/monitor/src/main/scala/monitor/parser/STParser.scala) extends Scala [`StandardTokenParsers`](https://www.scala-lang.org/api/2.12.2/scala-parser-combinators/scala/util/parsing/combinator/syntactical/StandardTokenParsers.html) to parse the provided session type and builds an abstract syntax tree consisting of the nodes found in the [`model`](https://github.com/chrisbartoloburlo/ecoop2021-artifact/tree/main/monitor/src/main/scala/monitor/model) directory. 
This tree is then passed to [`STInterpreter`](https://github.com/chrisbartoloburlo/ecoop2021-artifact/blob/main/monitor/src/main/scala/monitor/interpreter/STInterpreter.scala) which, while traversing the tree, makes use of the APIs offered by [`SynthMon`](https://github.com/chrisbartoloburlo/ecoop2021-artifact/blob/main/monitor/src/main/scala/monitor/synth/SynthMon.scala) and [`SynthProtocol`](https://github.com/chrisbartoloburlo/ecoop2021-artifact/blob/main/monitor/src/main/scala/monitor/synth/SynthProtocol.scala) to generate the Scala code of the monitor and CPSP classes respectively, depending on the current type (represented by the node).
