...
Where:
  ...
  * `$PREAMBLE` _(optional)_ is a file containing a preamble that will be added to the top of the generated files (typically containing package declarations and imports, as in [this file](https://github.com/chrisbartoloburlo/stmonitor/tree/master/examples/src/main/scala/examples/http/preamble.txt)).
...
The source code of all examples is available under the [`examples/`](https://github.com/chrisbartoloburlo/stmonitor/tree/master/examples/src/main/scala/examples/) directory.
...
The protocol is formalised as the session type `S_auth` below, as found in the file [`auth.st`](https://github.com/chrisbartoloburlo/stmonitor/tree/master/examples/src/main/scala/examples/auth/auth.st):
...
The functions `util.validateUname()` and `util.validateTok()` can be found in the file [`util.scala`](https://github.com/chrisbartoloburlo/stmonitor/tree/master/examples/src/main/scala/examples/auth/util.scala). For the sake of the example, their implementations always return `true`; you can change their return value to `false` and observe that the monitor indeed flags assertion violations.

The synthesised monitor [`Monitor.scala`](https://github.com/chrisbartoloburlo/stmonitor/tree/master/examples/src/main/scala/examples/auth/Monitor.scala) and CPSP classes [`CPSPc.scala`](https://github.com/chrisbartoloburlo/stmonitor/tree/master/examples/src/main/scala/examples/auth/CPSPc.scala) are generated automatically from the file [`auth.st`](https://github.com/chrisbartoloburlo/stmonitor/tree/master/examples/src/main/scala/examples/auth/auth.st), every time the example is compiled or executed via `sbt`.
...
#### Setup 1: untrusted client, trusted server written in Scala+lchannels
 ...
 The synthesised monitor ([`Monitor.scala`](https://github.com/chrisbartoloburlo/stmonitor/tree/master/examples/src/main/scala/examples/auth/Monitor.scala)) verifies the interaction between an untrusted client and a trusted server ([`Server.scala`](https://github.com/chrisbartoloburlo/stmonitor/tree/master/examples/src/main/scala/examples/auth/Server.scala)) implemented in Scala using the `lchannels` library.
 
 In this setup the monitor makes use of a [connection manager](https://github.com/chrisbartoloburlo/stmonitor/tree/master/examples/src/main/scala/examples/auth/ConnectionManager.scala), which sits between the untrusted client and the monitor: its purpose is to translate messages from a text-based protocol to the types present in the generated [`CPSPc.scala`](https://github.com/chrisbartoloburlo/stmonitor/tree/master/examples/src/main/scala/examples/auth/CPSPc.scala) file, and vice-versa.
...
In this setup, the synthesised monitor [`Monitor.scala`](https://github.com/chrisbartoloburlo/stmonitor/tree/master/examples/src/main/scala/examples/auth/Monitor.scala) (the same used in the previous setup) is used to verify the interaction between an untrusted client and a trusted server written in Python ([`auth-server.py`](https://github.com/chrisbartoloburlo/stmonitor/tree/master/scripts/auth-server.py)), hence *not* using `lchannels`.

In this setup the monitor makes use of *two* connection managers:

  * [one](https://github.com/chrisbartoloburlo/stmonitor/tree/master/examples/src/main/scala/examples/auth/ConnectionManager.scala) sits between the untrusted client and the monitor (it is the same used in the previous setup);
  * the [other](https://github.com/chrisbartoloburlo/stmonitor/tree/master/examples/src/main/scala/examples/auth/MonWrapper.scala#L12) sits between the monitor and the server, and provides a suitable message transport for the `lchannels` library. This connection manager also translates messages to/from the types found in the [`CPSPc.scala`](https://github.com/chrisbartoloburlo/stmonitor/tree/master/examples/src/main/scala/examples/auth/CPSPc.scala) file --- but it performs less run-time checks, since it is facing the trusted server.
...
The type `S_game` below can be found in [`game.st`](https://github.com/chrisbartoloburlo/stmonitor/tree/master/examples/src/main/scala/examples/game/game.st). In this case, the type describes the protocol from the server side.
```
S_game = rec X.(&{
...
We provide the implementation of a [client written in Scala](https://github.com/chrisbartoloburlo/stmonitor/tree/master/examples/src/main/scala/examples/game/MonitoredClient.scala), and a [server written in Python](https://github.com/chrisbartoloburlo/stmonitor/tree/master/scripts/game-server.py). 
...
2. In a separate terminal, you can launch a [`client`](https://github.com/chrisbartoloburlo/stmonitor/tree/master/examples/src/main/scala/examples/game/MonitoredClient.scala) that also spawns the [monitor synthesised from `S_game`](https://github.com/chrisbartoloburlo/stmonitor/tree/master/examples/src/main/scala/examples/game/Monitor.scala) (as a separate thread), by executing:
   ```shell
...
The session type `S_calc` below (taken from [`calc.st`](https://github.com/chrisbartoloburlo/stmonitor/tree/master/examples/src/main/scala/examples/calc/calc.st)) describes a simple calculator, that (recursively) lets user ask to `Add` two numbers, and answers with their sum.
```
...
The generated monitor uses a [`preamble`](https://github.com/chrisbartoloburlo/stmonitor/tree/master/examples/src/main/scala/examples/calc/preamble.txt) and the file [`util.scala`](https://github.com/chrisbartoloburlo/stmonitor/tree/master/examples/src/main/scala/examples/calc/util.scala) plementing the function `util.checkAdd()`, where the assertion function `util.checkAdd` is defined.
...
The generated files are [`Monitor.scala`](https://github.com/chrisbartoloburlo/stmonitor/tree/master/examples/src/main/scala/examples/calc/Monitor.scala) and [`CPSPc.scala`](https://github.com/chrisbartoloburlo/stmonitor/tree/master/examples/src/main/scala/examples/calc/CPSPc.scala).
...
The source code of all benchmarks is available under the [`examples/`](https://github.com/chrisbartoloburlo/stmonitor/tree/master/examples/src/main/scala/examples/) directory.
...
* `$EXPERIMENTS` is a space-separated list of the experiments to run. The choices are one or more of: 
    * `smtp-python`: a trusted client sends a number of emails to an untrusted SMTP server, implemented using [`smtpd`](https://docs.python.org/3/library/smtpd.html) from the Python standard library.  The monitor for this experiment is generated from a fragment of the SMTP protocol, formalised as a session type. Some relevant files for this benchmark are:
        * [`smtp.st`](https://github.com/chrisbartoloburlo/stmonitor/tree/master/examples/src/main/scala/examples/smtp/smtp.st): session type with SMTP protocol fragment
        * [`Monitor.scala`](https://github.com/chrisbartoloburlo/stmonitor/tree/master/examples/src/main/scala/examples/smtp/Monitor.scala) and [`CPSPc.scala`](https://github.com/chrisbartoloburlo/stmonitor/tree/master/examples/src/main/scala/examples/smtp/CPSPc.scala): files generated from the session type above
    ...
    * `pingpong`: a trusted server accepts 'ping' requests over HTTP, and answers 'pong'. The untrusted client is JMeter, configured to send an increasing number of requests and measure the server performance. Some relevant files for this benchmark are:
        * [`pingpong.st`](https://github.com/chrisbartoloburlo/stmonitor/tree/master/examples/src/main/scala/examples/pingpong/pingpong.st): session type for ping-pong protocol
        * [`Monitor.scala`](https://github.com/chrisbartoloburlo/stmonitor/tree/master/examples/src/main/scala/examples/pingpong/Monitor.scala) and [`CPSPc.scala`](https://github.com/chrisbartoloburlo/stmonitor/tree/master/examples/src/main/scala/examples/pingpong/CPSPc.scala): files generated from the session type above
    * `http`: a trusted web server implements a fragment of the HTTP protocol, formalised as the session type. The untrusted client is JMeter, configured to send an increasing number of requests and measure the server performance. Some relevant files for this benchmark are:
        * [`http.st`](https://github.com/chrisbartoloburlo/stmonitor/tree/master/examples/src/main/scala/examples/http/http.st): session type with HTTP protocol
        * [`Monitor.scala`](https://github.com/chrisbartoloburlo/stmonitor/tree/master/examples/src/main/scala/examples/http/Monitor.scala) and [`CPSPc.scala`](https://github.com/chrisbartoloburlo/stmonitor/tree/master/examples/src/main/scala/examples/http/CPSPc.scala): files generated from the session type above
        * [`Worker.scala`](https://github.com/chrisbartoloburlo/stmonitor/tree/master/examples/src/main/scala/examples/http/Worker.scala): core of the trusted web server, derived from the [`lchannels` HTTP server](https://github.com/alcestes/lchannels/tree/master/examples/src/main/scala/lchannels/examples/http).
